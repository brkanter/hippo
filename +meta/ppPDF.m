...
for iFolder = 1:length(allFolders)    
    ...
    if expType == 8 && (iFolder > 1 && iFolder < length(allFolders))
        posAve(:,2:3) = posAve(:,2:3) * (-1);
        spikesPos(:,2:3) = spikesPos(:,2:3) * (-1);
    end
    ...
    for iCluster = 1:numClusters          % loop through all cells in folder
        ...
        spikesPos = data.getSpikePositions(spikes,posAve);
...
%         meanRate = analyses.meanRate(spikes, posAve);
        posMat{iCluster,iFolder} = posAve;
        spikeMat{iCluster,iFolder} = spikesPos;    
...
for iSheet = 1:(length(allFolders)/numSesh)    
    ...
    numSheets(iSheet) = ceil(size(posMat(~cellfun(@isempty,posMat(:,counter))),1)/4);
...
for iExp = 1:(length(allFolders)/numSesh)  % every experiment
    ...
    for iSheet = 1:numSheets(iExp)   % every sheet for that experiment        
        ...
        for iPlotsY = 1:subplotsy           % each cell            
            ...
            if cellCount > size(posMat(~cellfun(@isempty,posMat(:,sessionCount))),1); break; end;            
            for iPlotsX = 1:subplotsx           % each session                
                ...
                if ~isempty(posMat{cellCount,sessionCount})    % make sure map exists                    
                    if scaleMethod == 1     % autoscale each session  
                        plot(posMat{cellCount,sessionCount}(:,2),posMat{cellCount,sessionCount}(:,3),'color',[0.5 0.5 0.5])
                        hold on
                        plot(spikeMat{cellCount,sessionCount}(:,2),spikeMat{cellCount,sessionCount}(:,3),'k.','markersize',15)
                        set(gca,'ydir','reverse')
                    elseif scaleMethod == 2  % scale to peak of first session                        
                        if ~isempty(posMat{cellCount,sessionCount-(iPlotsX-1)})    % make sure 1st map exists                            
                            prevMax = nanmax(nanmax(posMat{cellCount,sessionCount-(iPlotsX-1)}));  % find previous max                            
                            if prevMax == 0; prevMax = 0.0001; end;   % make sure prevMax isn't zero                            
                            if nanmin(nanmin(posMat{cellCount,sessionCount})) > prevMax   % if min is bigger than max, set min to zero                                
                                [scaleBar,~] = colorMapBRK(posMat{cellCount,sessionCount},'bar','on','pubQual',pubQual,'cutoffs',[0,prevMax]);                                
                            else                                
                                [scaleBar,~] = colorMapBRK(posMat{cellCount,sessionCount},'bar','on','pubQual',pubQual,'cutoffs',[nanmin(nanmin(posMat{cellCount,sessionCount})),prevMax]);                                
                            end                            
                        else                            
                            [scaleBar,~] = colorMapBRK(posMat{cellCount,sessionCount},'bar','on','pubQual',pubQual);   % autoscale cause 1st map doesn't exist                            
                        end                            
                    elseif scaleMethod == 3   % scale to peak of session with highest peak
                        ...
                        for iSession = (numSesh*iExp-(numSesh-1)):(numSesh*iExp)    % all sessions of current experiment                            
                            if ~isempty(posMat{cellCount,iSession})                                
                                allPeaks(iSession) = nanmax(nanmax(posMat{cellCount,iSession}));                                
                            end                            
                        end                        
                        maxPeak = nanmax(allPeaks);    % find max peak across all sessions                        
                        if maxPeak == 0; maxPeak = 0.0001; end;   % make sure max isn't zero                        
                        if nanmin(nanmin(posMat{cellCount,sessionCount})) > maxPeak   % if min is bigger than max, set min to zero                            
                            [scaleBar,~] = colorMapBRK(posMat{cellCount,sessionCount},'bar','on','pubQual',pubQual,'cutoffs',[0,maxPeak]);                            
                        else                            
                            [scaleBar,~] = colorMapBRK(posMat{cellCount,sessionCount},'bar','on','pubQual',pubQual,'cutoffs',[nanmin(nanmin(posMat{cellCount,sessionCount})),maxPeak]);                            
                        end                        
                    elseif scaleMethod == 4    % scale to peak As to A and Bs to B                        
                        switch iPlotsX                            
                            case {1,2}                                
                                [scaleBar,~] = colorMapBRK(posMat{cellCount,sessionCount},'bar','on','pubQual',pubQual);  % autoscale                                
                            case {3,4}                                
                                if ~isempty(posMat{cellCount,sessionCount-2})    % make sure old map exists                                    
                                    prevMax = nanmax(nanmax(posMat{cellCount,sessionCount-2}));   % scale to 2 sessions ago                                    
                                    ...
                                    [scaleBar,~] = colorMapBRK(posMat{cellCount,sessionCount},'bar','on','pubQual',pubQual);   % autoscale cause old map doesn't exist                                    
                                end                                    
                            case {5,6}                                                                    
                                if ~isempty(posMat{cellCount,sessionCount-4})    % make sure old map exists                                    
                                    prevMax = nanmax(nanmax(posMat{cellCount,sessionCount-4}));   % scale to 4 sessions ago                                    
                                    if prevMax == 0; prevMax = 0.0001; end;  % make sure max isn't zero                                    
                                    if nanmin(nanmin(posMat{cellCount,sessionCount})) > prevMax    % if min is bigger than max, set min to zero                                        
                                        [scaleBar,~] = colorMapBRK(posMat{cellCount,sessionCount},'bar','on','pubQual',pubQual,'cutoffs',[0,prevMax]);                                        
                                    else                                        
...
%                 switch expType                    
%                     case {1,7,8}
%                         if ~isempty(posMat{cellCount,sessionCount})
%                             Q = qualityMat{cellCount,sessionCount};
%                             if strcmpi(Q,'4')
%                                 Q = 'off';
%                             end
%                             bottomTitle = title(sprintf('T%dC%d  Q%s  m=%.2f',tetrodeMat{cellCount,sessionCount},clusterMat{cellCount,sessionCount},Q,spikeMat{cellCount,sessionCount}));
...
%                     case {2,3,5,6}
%                         if ~isempty(posMat{cellCount,sessionCount})
%                             bottomTitle = title(sprintf('T%dC%d  [%.2f]  [%.2f]',tetrodeMat{cellCount,sessionCount},clusterMat{cellCount,sessionCount},spikeMat{cellCount,sessionCount},peakMat{cellCount,sessionCount}));
