...
% Extract column or columns from array after filtering for certain info.
%
%   USAGE
%       colOut = getColComp(array,labels,colToGet,varagin)
%       array          cell array of data
%       labels         cell array of strings containing column headers
%       colsToGet      string label of column to extract, or ':' for all
...
%
%   OUTPUT
%       colOut         extracted column or columns from array
%
%   SEE ALSO
%       getCol deleteRowsComp
%
% Written by BRK 2016

function colOut = getColComp(array,labels,colsToGet,varargin)

%% inspect columns
if ~strcmpi(colsToGet,':')
    columns = strcmpi(colsToGet,labels);
else
    columns = ':';
end
...
colOut = eval(sprintf('array(%s,%s)','totalLog','columns'));
if length(colOut) == 1
    colOut = colOut{1};
end