...
function pvCorr = populationVectorCorrelation(stack1, stack2, varargin)
    ...
    for i = 1:2:length(varargin)
        if ~ischar(varargin{i})
            error(['Parameter ' num2str(i+2) ' is not a property (type ''help <a href="matlab:help analyses.populationVectorCorrelation">analyses.populationVectorCorrelation</a>'' for details).']);
        end

        switch(lower(varargin{i}))
            case 'full'
                %returnFull = strcmpi(varargin{i+1}, 'on');
                if strcmpi(varargin{i+1}, 'on')
                    returnFormat = returnFormats.D3;
                elseif strcmpi(varargin{i+1}, 'vector')
                        returnFormat = returnFormats.D1;
                end

            case 'orientation'
                isOrientationX = strcmpi(varargin{i+1}, 'h');

            otherwise
                error(['Unknown property ''' num2str(varargin{i}) ''' (type ''help <a href="matlab:help analysis.populationVectorCorrelation">analysis.populationVectorCorrelation</a>'' for details).']);
        end
    end
    ...
    if numCells1 ~= numCells2
        warning('Population vectors are of different size: %u vs %u', numCells1, numCells2);
    ...
    for i = 1:numBins % it doesn't matter whether we iterate over y or x bins
        ...
        switch returnFormat
            case returnFormats.D3
                pvCorr(:, :, i) = corr(reshapedLeft, reshapedRight);
            ...
            case returnFormats.D2
                ...
                reshapedRight(nanL | nanR) = 0;
                %%
                
                pvCorr(i, :) = diag(corr(reshapedLeft, reshapedRight));
            ...
            case returnFormats.D1
                if isOrientationX
                    pvCorr(1, i) = corr(reshapedLeft', reshapedRight');
                else
                    pvCorr(i, 1) = corr(reshapedLeft', reshapedRight');
                end
        end
    end
end
