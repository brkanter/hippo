...
% emperorPenguin: batching function for neurobiological analysis of spatial behavior.
%
...
%                 load(sprintf('TT%d_%d-CluQual.mat',cellMatrix(iCluster,1),cellMatrix(iCluster,2)))
        ...
        if include_HD
            ...
            try
                ...
                [~,~,meanAngle] = circ_plotBRK(HDrad,'pretty','r.',false);
                close;
                vLength = circ_r(HDrad);
                if isempty(meanAngle); meanAngle = 999999; end;
            catch
                vLength = 999999;
                meanAngle = 999999;
            end
